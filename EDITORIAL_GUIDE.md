...
`published_date` is machine-readable metadata and, when present, must be an exact calendar date in `YYYY-MM-DD` format. Do not append prose such as `issue`, `edition`, `approx.`, or a season to this field. Put explanatory date context in prose or another optional metadata field instead. Before publication, validate every populated `published_date` against `^\\d{4}-\\d{2}-\\d{2}# Morning Edition — Editorial Guide
...
. An invalid story date is a publication failure because display code may consume it as a real date.
